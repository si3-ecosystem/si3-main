...
function AccordionTrigger({
  className,
  children,
  showPurpleChevron = false,
  showChevronRight = false,
  ...props
}: React.ComponentProps<typeof AccordionPrimitive.Trigger> & {
  showPurpleChevron?: boolean;
    ...
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ...
        className={cn(
          ...
          !showChevronRight && "[&[data-state=open]>svg]:rotate-180", // Only apply rotation if showChevronRight is false
          className,
        )}
        {...props}
      >
        ...
        {showChevronRight ? (
          <ChevronRightIcon
            className={cn(
              "text-muted-foreground pointer-events-none size-6 w-fit shrink-0 translate-y-0.5 transition-transform duration-200",
              showPurpleChevron && "text-primary",
            )}
          />
        ) : (
          <ChevronDownIcon
